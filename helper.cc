...
//added sem_timedwait function
//timeout parameter specifies a limit on the time (timeout) that the call should block if
//the wait cannot be performed immediately. if the semaphore cannot be locked
//by the time timeout expires, the wait will be terminated when the specified time expires.
int sem_timedwait (int id, short unsigned int num, const struct timespec *timeout)
{
  struct sembuf op[] = {
    {num, -1, SEM_UNDO}
  };
  return semtimedop (id, op, 1, timeout); //function must return an int
}
